...
def get_kline_etf_from_realtime_df():
    """从realtime_etf_df表查询数据"""
    sql = """
    SELECT 
        rss.date,
        rss.date_int,
    ...
    FROM 
        `realtime_etf_df` rss
    WHERE
        rss.date_int = (SELECT MAX(date_int) FROM `realtime_etf_df`)
        AND rss.code IN (
            SELECT code 
            FROM `basic_info_etf` 
            WHERE `参与指标计算` = 1
    ...
    """
    print("正在从realtime_etf_df查询指数数据...")
    df = DBManager.query_sql(sql)
    print(f"查询完成，获取到 {len(df)} 条记录")
    return df


def get_kline_etf_from_realtime_sina():
    """从realtime_etf_sina表查询数据（添加turnoverrate字段）"""
    sql = """
    SELECT 
        rss.date,
        rss.date_int,
        ...
        rss.`成交量(手)` AS volume
    FROM 
        `realtime_etf_sina` rss
    WHERE
        rss.date_int = (SELECT MAX(date_int) FROM `realtime_etf_sina`)
        AND rss.code IN (
            SELECT code 
            FROM `basic_info_etf` 
            WHERE `参与指标计算` = 1
    ...
    """
    print("正在从realtime_etf_sina查询股票数据...")
    df = DBManager.query_sql(sql)
    print(f"查询完成，获取到 {len(df)} 条记录")
    return df
...
def main():
    ...
    # 第一步：从realtime_etf_df表获取数据并插入
    df_df = get_kline_etf_from_realtime_df()
    if not df_df.empty:
        # 生成批量SQL
        sql_batches_df = sql_batch_generator(
            table_name='kline_etf',
            data=df_df,
            batch_size=6000
        )
        # 执行批量插入
        ...
        print(f"[Success]realtime_etf_df数据写入完成，数据量：{len(df_df)}")
    else:
        print("未获取到realtime_etf_df数据")

    # 第二步：从realtime_etf_sina表获取数据并插入
    df_sina = get_kline_etf_from_realtime_sina()
    if not df_sina.empty:
        ...
        print(f"[Success]realtime_etf_sina数据写入完成，数据量：{len(df_sina)}")
    else:
        print("未获取到realtime_etf_sina数据")
