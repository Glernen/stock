    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # 执行批量插入
                execute_batch_sql(sql_batches)
                print(f"[Success] 股票实时表写入完成，数据量：{len(temp_df)}")
            except Exception as e:
                print(f"[Critical] 股票实时表写入失败: {str(e)}")
        except Exception as e:
            print(f"[Error] 股票实时表写入失败: {e}")
        #################################################
        ...
        try:
            ...
            try:
                ...
                print(f"[Critical] 股票基础表写入失败: {str(e)}")
        except Exception as e:
            print(f"[Error] 股票基础表写入失败: {e}")


        # ==== 历史信息表（cn_stock_hist_daily）写入逻辑 ====
        ...
        try:
            ...
            stock_data_df.loc[:, "volume"] = temp_df["volume"] #成交量
            stock_data_df.loc[:, "turnover"] = temp_df["turnoverrate"] #turnover
            stock_data_df.loc[:, "amount"] = temp_df["deal_amount"] #成交额
            stock_data_df.loc[:, "amplitude"] = temp_df["amplitude"] #振幅
            stock_data_df.loc[:, "quote_change"] = temp_df["change_rate"] #涨跌幅
            stock_data_df.loc[:, "ups_downs"] = temp_df["ups_downs"] #涨跌额
            stock_data_df.loc[:, "period"] = "daily" #周期

            # print(f'历史数据表样：{stock_data_df}')
            try:
                # 生成批量SQL
                sql_batches = sql_batch_generator(
                ...
                # 执行批量插入
                execute_batch_sql(sql_batches)
                print(f"[Success] 股票历史信息表写入完成，数据量：{len(stock_data_df)}")
            except Exception as e:
                print(f"[Critical] 股票历史信息表写入失败: {str(e)}")
        except Exception as e:
            print(f"[Error] 股票历史信息表写入失败: {e}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(f"[Success] 基金基础信息表写入完成，数据量：{len(etf_info_df)}")
            except Exception as e:
                print(f"[Critical] 基金基础信息表写入失败: {str(e)}")
        except Exception as e:
            print(f"[Error] 基金基础信息表写入失败: {e}")
        ################################################


        # ==== 历史信息表（cn_etf_hist_daily）写入逻辑 ====
        ...
        try:
            ...
            etf_data_df.loc[:, "volume"] = temp_df["volume"] #成交量
            etf_data_df.loc[:, "amount"] = temp_df["deal_amount"] #成交额
            etf_data_df.loc[:, "amplitude"] = temp_df["amplitude"] #振幅
            etf_data_df.loc[:, "quote_change"] = temp_df["change_rate"] #涨跌幅
            etf_data_df.loc[:, "ups_downs"] = temp_df["ups_downs"] #涨跌额
            etf_data_df.loc[:, "turnover"] = temp_df["turnoverrate"] #换手率
            etf_data_df.loc[:, "period"] = "daily" #周期
            # print(f'历史数据表样：{etf_data_df}')
            try:
                # 生成批量SQL
                sql_batches = sql_batch_generator(
                ...
                print(f"[Success] 基金历史信息表写入完成，数据量：{len(etf_data_df)}")
            except Exception as e:
                print(f"[Critical] 基金历史信息表写入失败: {str(e)}")

        except Exception as e:
            print(f"[Error] 基金历史信息表写入失败: {e}")
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # 执行批量插入
                execute_batch_sql(sql_batches)
                print(f"[Success] 指数实时表写入完成，数据量：{len(temp_df)}")
            except Exception as e:
                print(f"[Critical] 指数实时表写入失败: {str(e)}")
        except Exception as e:
            print(f"[Error] 指数实时表写入失败: {e}")
        #################################################


         # ==== 基础信息表（cn_stock_spot）写入逻辑 ====
        ...
        # ==== 历史信息表（cn_index_hist_daily）写入逻辑 ====
        ...
        try:
            ...
            index_data_df.loc[:, "amount"] = temp_df["deal_amount"] #成交额
            index_data_df.loc[:, "amplitude"] = temp_df["amplitude"] #振幅
            index_data_df.loc[:, "quote_change"] = temp_df["change_rate"] #涨跌幅
            index_data_df.loc[:, "ups_downs"] = temp_df["ups_downs"] #涨跌额
            index_data_df.loc[:, "turnover"] = temp_df["turnoverrate"] #换手率
            index_data_df.loc[:, "period"] = "daily" #周期
            # print(f'历史数据表样：{etf_data_df}')
            try:
                # 生成批量SQL
                sql_batches = sql_batch_generator(
                ...
                print(f"[Success] 指数历史信息表写入完成，数据量：{len(index_data_df)}")
            except Exception as e:
                print(f"[Critical] 指数历史信息表写入失败: {str(e)}")

        except Exception as e:
            print(f"[Error] 指数历史信息表写入失败: {e}")
