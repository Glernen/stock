...
def get_index_basic_info():
    ...
    SELECT
      ...
      (
        CASE
          WHEN (lower(`rss`.`symbol`) LIKE 'sh%') THEN
            1
          WHEN (lower(`rss`.`symbol`) LIKE 'sz%') THEN
            0
          ELSE
            COALESCE(`rsd`.`市场标识`, '')
        END
      ...
      (
        CASE
          WHEN (lower(`rss`.`symbol`) LIKE 'sh%') THEN
            concat('1.', lpad(`rss`.`code`, 6, '0'))
          WHEN (lower(`rss`.`symbol`) LIKE 'sz%') THEN
            concat('0.', lpad(`rss`.`code`, 6, '0'))
          ELSE
            concat(COALESCE(`rsd`.`市场标识`, ''), '.', lpad(`rss`.`code`, 6, '0'))
        END
      ) AS `市场代码`,
      `rss`.`symbol` AS `symbol`
    FROM
      (
        `realtime_index_sina` `rss`
        LEFT JOIN `realtime_index_df` `rsd` ON (((`rss`.`code_int` = `rsd`.`code_int`) AND (`rss`.`date_int` = `rsd`.`date_int`)))
      )
    WHERE
      (`rss`.`date_int` = (SELECT max(`realtime_index_sina`.`date_int`) FROM `realtime_index_sina`))
    """
...
def main():
    ...
    # 获取股票基础信息
    basic_info_index = get_index_basic_info()

    if basic_info_index.empty:
        print("未获取到股票基础信息数据，程序终止")
        return

    # 生成批量SQL
    sql_batches = sql_batch_generator(
        table_name='basic_info_index',
        data=basic_info_index,
        batch_size=6000  # 根据实际情况调整
    )

    # 执行批量插入
    execute_batch_sql(sql_batches)
    print(f"[Success]股票基础信息表数据写入完成，数据量：{len(basic_info_index)}，耗时 {time.time() - start_time:.2f}秒")
