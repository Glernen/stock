...
def get_kline_stock_from_realtime_df():
    ...
    FROM 
        `realtime_stock_df` rss
    WHERE
        rss.date_int = (SELECT MAX(date_int) FROM `realtime_stock_df`)
        AND rss.code IN (
            SELECT code 
            FROM `basic_info_stock` 
            WHERE `参与指标计算` = 1
        );
    """
    print("正在从realtime_stock_df查询股票数据...")
    df = DBManager.query_sql(sql)
    print(f"查询完成，获取到 {len(df)} 条记录")
    return df

def get_kline_stock_from_realtime_sina():
    ...
    """
    print("正在从realtime_stock_sina查询股票数据...")
    df = DBManager.query_sql(sql)
    print(f"查询完成，获取到 {len(df)} 条记录")
    return df
...
def main():
    ...
    df_df = get_kline_stock_from_realtime_df()
    if not df_df.empty:
        # 生成批量SQL
        ...
        print(f"[Success]realtime_stock_df数据写入完成，数据量：{len(df_df)}")
    else:
        print("未获取到realtime_stock_df数据")

    # 第二步：从realtime_stock_sina表获取数据并插入
    df_sina = get_kline_stock_from_realtime_sina()
    if not df_sina.empty:
        ...
        print(f"[Success]realtime_stock_sina数据写入完成，数据量：{len(df_sina)}")
    else:
        print("未获取到realtime_stock_sina数据")
