...
def get_etf_basic_info():
    ...
    FROM
      (
        ...
        LEFT JOIN `realtime_etf_df` `rsd` ON (((`rss`.`code_int` = `rsd`.`code_int`) AND (`rss`.`date_int` = `rsd`.`date_int`)))
      )
    WHERE
      (`rss`.`date_int` = (SELECT max(`realtime_etf_sina`.`date_int`) FROM `realtime_etf_sina`))
    """
...
def main():
    ...
    # 获取股票基础信息
    basic_info_etf = get_etf_basic_info()

    if basic_info_etf.empty:
        print("未获取到股票基础信息数据，程序终止")
        return

    # 生成批量SQL
    sql_batches = sql_batch_generator(
        table_name='basic_info_etf',
        data=basic_info_etf,
    ...
    print(f"[Success]股票基础信息表数据写入完成，数据量：{len(basic_info_etf)}，耗时 {time.time() - start_time:.2f}秒")
