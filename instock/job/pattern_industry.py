...
def get_industry_data(date_int):
    """获取指定日期的行业数据"""
    sql = f"""
    SELECT *
    FROM kline_industry 
...
def process_industry_data(industry_data):
    """处理行业数据并识别锤子线形态"""
    # 转换为DataFrame
...
def save_to_database(df):
    ...
    # 生成批量SQL
    sql_batches = sql_batch_generator('pattern_industry', filtered_df, 6000)
...
def main():
    ...
    # 获取该日期的行业数据
    industry_data = get_industry_data(latest_date)
    if industry_data.empty:
        print(f"未找到 {latest_date} 的行业数据")
        return

    print(f"获取到 {len(industry_data)} 条行业数据")

    # 处理数据并识别形态
    result_df = process_industry_data(industry_data)
    ...
    print(f"识别到 {bullish_count} 只行业出现阳锤子线形态")
    print(f"识别到 {bearish_count} 只行业出现阴锤子线形态")
