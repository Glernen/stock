...
# ETF历史数据配置
CN_ETF_HIST_DAILY_DATA = create_hist_data('cn_etf', 'daily', '日')
...
def fetch_single_hist(code: str, market_id: str, period: str, name: str, data_type: str, beg: str, end: str):
    ...
    try:
        ...
        if not data_json.get("data"):
            print(f"[Warning] {code} 无数据返回，可能已退市或停牌")
            return None
        ...
        df['code'] = code
        df['code_int'] = int(code) if code.isdigit() else 0
        df['period'] = period
        df['name'] = name
...
def fetch_all_stock_hist(beg: str = None, end: str = None):
    ...
    # 获取股票列表
    conn = DBManager.get_new_connection()
    stock_df = pd.read_sql("SELECT code, market_id, name FROM cn_stock_info WHERE date = (SELECT MAX(date) FROM cn_stock_info) AND market_id IS NOT NULL", conn)
    conn.close()
    ...
    # 单线程获取数据（添加进度条）
    for idx, row in tqdm(stock_df.iterrows(), total=len(stock_df), desc="获取股票历史数据"):
        code = row['code']
        market_id = row['market_id']
        name = row['name']

        data = fetch_single_hist(code, market_id, "daily", name, "stock", beg, end)
...
def fetch_all_etf_hist(beg: str = None, end: str = None):
    ...
    # 获取ETF列表
    conn = DBManager.get_new_connection()
    etf_df = pd.read_sql("SELECT code, market_id, name FROM cn_etf_info WHERE date = (SELECT MAX(date) FROM cn_etf_info) AND market_id IS NOT NULL", conn)
    conn.close()

    print(f"待采集ETF数量：{len(etf_df)}")

    # 准备数据容器
    daily_data = pd.DataFrame()

    # 单线程获取数据（添加进度条）
    for idx, row in tqdm(etf_df.iterrows(), total=len(etf_df), desc="获取ETF历史数据"):
        code = row['code']
        market_id = row['market_id']
        name = row['name']

        data = fetch_single_hist(code, market_id, "daily", name, "etf", beg, end)
    ...
    print(f"[Success] ETF历史数据写入完成（日：{len(daily_data)}）")
...
def fetch_all_index_hist(beg: str = None, end: str = None):
    ...
    # 获取指数列表
    conn = DBManager.get_new_connection()
    index_df = pd.read_sql("SELECT code, market_id, name FROM cn_index_info WHERE date = (SELECT MAX(date) FROM cn_index_info) AND market_id IS NOT NULL", conn)
    conn.close()

    print(f"待采集指数数量：{len(index_df)}")

    # 准备数据容器
    daily_data = pd.DataFrame()

    # 单线程获取数据（添加进度条）
    for idx, row in tqdm(index_df.iterrows(), total=len(index_df), desc="获取指数历史数据"):
        code = row['code']
        market_id = row['market_id']
        name = row['name']

        data = fetch_single_hist(code, market_id, "daily", name, "index", beg, end)
...
def main():
    ...
    # 场景2: 单日期模式
        ...
        try:
            ...
            print(f"开始采集指定日期行情数据: {clean_date}")
    ...
    # 场景3: 日期区间模式
        ...
        try:
            ...
            print(f"开始采集日期区间行情数据: {beg_str} 至 {end_str}")
