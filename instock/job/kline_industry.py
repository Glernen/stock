...
def get_kline_industry_from_realtime_df():
    """从realtime_industry_df表查询数据"""
    sql = """
    SELECT 
        rss.date,
        rss.date_int,
    ...
    FROM 
        `realtime_industry_df` rss
    WHERE
        rss.date_int = (SELECT MAX(date_int) FROM `realtime_industry_df`)
        AND rss.code IN (
            SELECT code 
            FROM `basic_info_industry` 
            WHERE `参与指标计算` = 1
        );
    """
    print("正在从realtime_industry_df查询指数数据...")
...
def main():
    ...
    df_df = get_kline_industry_from_realtime_df()
    if not df_df.empty:
        # 生成批量SQL
        sql_batches_df = sql_batch_generator(
            table_name='kline_industry',
            data=df_df,
            batch_size=6000
        )
        # 执行批量插入
        ...
        print(f"[Success]kline_industry数据写入完成，数据量：{len(df_df)}")
    else:
        print("未获取到realtime_industry_df数据")
