...
def save_to_database(df):
    ...
    # 生成批量SQL
    sql_batches = sql_batch_generator('pattern_etf', filtered_df, 6000)
...
def main():
    ...
    if stock_data.empty:
        print(f"未找到 {latest_date} 的基金数据")
        return

    print(f"获取到 {len(stock_data)} 条基金数据")
    ...
    print(f"识别到 {bullish_count} 只基金出现阳锤子线形态")
    print(f"识别到 {bearish_count} 只基金出现阴锤子线形态")
