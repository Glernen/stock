...
def create_tables():
    ...
    CREATE TABLE IF NOT EXISTS `kline_index` (
        ...
        INDEX `idx_date` (`date`),
        INDEX `idx_code` (`code_int`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='指数K线数据表';
    """

    # 基金K线表
    kline_etf_sql = """
    CREATE TABLE IF NOT EXISTS `kline_etf` (
        `id` INT AUTO_INCREMENT PRIMARY KEY COMMENT '自增ID',
        `date` DATE NOT NULL COMMENT '日期',
        `date_int` INT NOT NULL COMMENT '日期整数格式',
        ...
        INDEX `idx_date` (`date`),
        INDEX `idx_code` (`code_int`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='基金K线数据表';
    """

    # 行业K线表(东方财富网)
    kline_industry_sql = """
    CREATE TABLE IF NOT EXISTS `kline_industry` (
        `id` INT AUTO_INCREMENT PRIMARY KEY COMMENT '自增ID',
        `date` DATE NOT NULL COMMENT '日期',
        `date_int` INT NOT NULL COMMENT '日期整数格式',
        ...
        INDEX `idx_date` (`date`),
        INDEX `idx_code` (`code_int`)
    ) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='申万1行业K线数据表';
    """

    # 股票基础信息表
    DBManager.execute_sql(kline_stock_sql)
    # 指数基础信息表
    DBManager.execute_sql(kline_index_sql)
    # 基金基础信息表
    DBManager.execute_sql(kline_etf_sql)
    # 行业基础信息表
    DBManager.execute_sql(kline_industry_sql)
