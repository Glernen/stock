...
def index_spot_sina():
    ...
    # 准备新浪实时指数数据
    realtime_index_sina = pd.DataFrame({
        "date": temp_df["date"],
    ...
    # print(realtime_index_sina.head())

    # 生成批量SQL
    sql_batches = sql_batch_generator(
        table_name='realtime_index_sina',
        data=realtime_index_sina,
        batch_size=6000  # 根据实际情况调整
    )

    # 执行批量插入
    execute_batch_sql(sql_batches)
    print(f"[Success] 新浪指数实时数据写入完成，数据量：{len(realtime_index_sina)}，耗时 {time.time() - start_time:.2f}秒")
    return realtime_index_sina
...
def index_spot_df() -> pd.DataFrame:
    '''
    东方财富网-沪深京 A 股-实时行情
    ...
    try:
        ...
        # 准备东方财富实时股票数据
        realtime_index_df = pd.DataFrame({
            "date": temp_df["date"],
            "date_int": temp_df["date_int"],
            "code": temp_df["f12"],
        ...
        # print(realtime_index_df.head())

        # 生成批量SQL
        sql_batches = sql_batch_generator(
            table_name='realtime_index_df',
            data=realtime_index_df,
            batch_size=6000  # 根据实际情况调整
        )
        ...
        execute_batch_sql(sql_batches)
        print(
            f"[Success] 东方财富指数实时数据写入完成，数据量：{len(realtime_index_df)}，耗时 {time.time() - start_time:.2f}秒")

        return realtime_index_df
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=3) as executor:
        # 提交三个任务到进程池
        futures = [
            executor.submit(index_spot_df),
            executor.submit(index_spot_sina)
