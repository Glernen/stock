...
def get_industry_basic_info():
    ...
    SELECT
          ...
          `rss`.`name` AS `name`,
          `市场标识`,
          concat(COALESCE(`市场标识`, ''), '.', lpad(`rss`.`code`, 6, '0')) AS `市场代码`,
          `rss`.`symbol` AS `symbol`
    FROM
        `realtime_industry_df` `rss`
    WHERE
        `rss`.`date_int` = (SELECT max(`realtime_industry_df`.`date_int`) FROM `realtime_industry_df`);
    """
...
def main():
    ...
    # 获取股票基础信息
    basic_info_industry = get_industry_basic_info()

    if basic_info_industry.empty:
        print("未获取到股票基础信息数据，程序终止")
        return

    # 生成批量SQL
    sql_batches = sql_batch_generator(
        table_name='basic_info_industry',
        data=basic_info_industry,
        batch_size=6000  # 根据实际情况调整
    )

    # 执行批量插入
    execute_batch_sql(sql_batches)
    print(f"[Success]行业基础信息表数据写入完成，数据量：{len(basic_info_industry)}，耗时 {time.time() - start_time:.2f}秒")
